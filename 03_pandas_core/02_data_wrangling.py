...
# Select by name
type(df[['bikeshop_name', 'city','state', 'frame_material']])

df[['bikeshop_name', 'city', 'state', 'frame_material']]
...
df.iloc[:,0:3] # all rows for the columns 0 to 3

df.iloc[:, -3:] # The last 3 columns


# Select by text matching

# ^ searches the from of a text string
df.filter(regex="(^model)|(^cat)", axis=1) 

#$  searched the back of a text string
df.filter(regex="(price$)|(date$)", axis=1)

# Rearranging columns

## Training the single approach
# Here I am pretty much rearranging the columns that are in the df
...
## - here I can do the multiple approach
...
df[['model', 'category_1','category_2', *k]]


# its first converted to a list
l = df.columns.tolist()
...
df[[*cols_to_front, *l2]]  # be critical of this code

# Select by data types

df.info()

# this grabs all the columns of object
df.select_dtypes(include="object") 

df.select_dtypes(include = "object")

df.select_dtypes(include ='int') # here we can make a selection only by the integer
...
# this grabs all the none string columns(without object type)
df2 = df.select_dtypes(exclude = "object")

df4 = df.select_dtypes(exclude= 'object')

# Here were are using the the columns selected by datatype inorder to rearrange 
pd.concat([df1, df2], axis = 1)

pd.concat([df3, df4], axis = 1)


# Moving columns to the front WITH DATAFRAME SELECTION


df7 = df[['model', 'category_1', 'category_2']]
type(df7)

df1 = df[['model', 'category_1', 'category_2']]
type(df1)

df8 = df.drop(['model','category_1','category_2'], axis=1)
...
# anything that starts with Trigger
...
# ~ this negates. i.e gives the inverse or opposite
...
# Top / Bottom(n-largest and n-smallest)
...
df.sample(n = 10, random_state=123) #due to random_state = 123, it will always return the same 10rows
...
df.select_dtypes(include= 'number').aggregate([np.mean, np.std])
...
summary_df_3.columns

# corrected datta wrangle and plot
summary_df_3\
    .assign(z_score = summary_df_3.groupby('category_2')['total_price']
    ...
    .plot(title="Z-Score of Total Price by Category Over Time")
...
summary_df_3\
    .assign(
        z_total_price = summary_df_3.groupby('category_2')['total_price']
        .transform(lambda x: (x - x.mean()) / x.std())
    )
...
summary_df_2\
    .rename(columns = dict(category_1 = "catgory 1"))
...
bikeshop_revenue_save = bikeshop_revenue_wide_df\
        .sort_values('Mountain', ascending=False)\
        .style\
        .highlight_max()\
        .format(
...
ggplot(
    mapping = aes(x = "Bikeshop Name", y = "Revenue", fill = "Category_1"),
...
sales_by_cat1_cat2_year_df = sales_by_cat1_cat2_year_df.T
...
sales_cat2_daily_df = sales_cat2_daily_df.drop('category_2', axis=1)
...
#Here we are calculating the mean while grouping by Category_2
sales_cat2_daily_df\
    .groupby('category_2')\
    .apply(np.mean)
...
#this is the best option since the date can be kept
